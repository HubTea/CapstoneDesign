...
export class ConfigListener implements BridgeListener {
  constructor(
    ...
    const configFile = fs.readFileSync('../../CapstoneConfig/config.json');
    const rawConfig = JSON.parse(configFile.toString());
    const validation = configSchema.validate(rawConfig);

    if (validation.error) {
      throw new Error(`${validation}`);
    }

    const config = validation.value as Config;
