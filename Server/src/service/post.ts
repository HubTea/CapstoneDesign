...
import { PostAttribute, CommentAttribute, Post } from '../model';
...
export class PostService {
  ...
  async register(
    ...
    const category = await this.getCategory(
      repository,
      postCreationDto.category,
    );

    const writer = await this.userService.getUserByAccount(repository, account);
  ...
  async getList(
    ...
    const listLength = 30;
    const limit = listLength + 1;
    const where: sequelize.WhereOptions<PostAttribute> = {};

    if (typeof categoryLabel == 'string') {
      const category = await this.getCategory(repository, categoryLabel);
    ...
    const rawList = await repository.post.findAll({
      where: where,
    ...
    const listDto = new ListDto<SimplePostDto>();
  ...
  async writeComment(
    ...
    const user = await this.userService.getUserByAccount(repository, account);
  ...
  async getCommentList(
    ...
    const listLength = 30;
    const limit = listLength + 1;
    const where: sequelize.WhereOptions<CommentAttribute> = {
      postId: postId,
    };
    ...
    const rawList = await repository.comment.findAll({
      where: where,
    ...
    const listDto = new ListDto<CommentDto>();
    let bound = rawList.length;
  ...
  private async getCategory(repository: RepositoryCollection, label: string) {
    const category = await repository.category.findOne({
      where: {
        label: label,
      },
    });
